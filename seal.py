...
aquisition_size = 1000
st_model = 'sentence-transformers/bert-base-nli-mean-tokens' #@param ['paraphrase-mpnet-base-v2', 'all-mpnet-base-v1', 'all-mpnet-base-v2', 'stsb-mpnet-base-v2', 'all-MiniLM-L12-v2', 'paraphrase-albert-small-v2', 'all-roberta-large-v1']
...
def fit(train_label,train_unlabel,algo="Entropy"):
    ...
    #check what learning rate should be given
    X_embedded = TSNE(n_components=2, init='random').fit_transform(X_train)
    plt.scatter(X_embedded[:,0], X_embedded[:, 1],color=['c' if i==1 else 'r' for i in y_train])
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.show()
